...
func (c *Config) RequestToken(postForm url.Values) (requestToken, requestSecret string, err error) {
	req, err := http.NewRequest("POST", c.Endpoint.RequestTokenURL, strings.NewReader(postForm.Encode()))
	if err != nil {
		return "", "", err
	}
